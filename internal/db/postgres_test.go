...
// TestDB holds test database connection and metadata
type TestDB struct {
	Name      string
	DB        *sql.DB
	ConnStr   string
...
// SetupTestDB creates a new database with a random name and applies the schema.
...
// GetTestPostgresDB creates a PostgresDB instance connected to the test database
func (tdb *TestDB) GetTestPostgresDB() (*PostgresDB, error) {
	return NewPostgresDB(tdb.ConnStr, 10, 5)
}
...
func TestUsageExample(t *testing.T) {
	...
	_, err := testDB.GetTestPostgresDB()
	if err != nil {
