// Package stochastic_heikin_ashi
package stochastic_heikin_ashi
...
func (s *StochasticHeikinAshi) OnCandles(ctx context.Context, oneHourCandles []candle.Candle) (signal.Signal, error) {
	...
	case NoPositionState:
		...
		if s.isStochasticOversold() && kValue > dValue && s.isHeikinAshiBullish(ha) {
			s.stateMachine.TransitionTo(WaitingForExitSignal, "stochastic_oversold_bullish_crossover_bullish_ha", position.LongBullish, "long signal triggered")
			return signal.Signal{
				Time:         lastCandle.Timestamp,
	...
	case WaitingForExitSignal:
		// Check for sell signal: stochastic overbought + bearish crossover
		if s.isStochasticOverbought() && kValue < dValue {
			s.stateMachine.TransitionTo(NoPositionState, "stochastic_overbought_bearish_crossover", position.LongBearish, "exit signal triggered")
			return signal.Signal{
				Time:         lastCandle.Timestamp,
