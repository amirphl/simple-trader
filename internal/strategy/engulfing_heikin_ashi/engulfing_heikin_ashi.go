...
func (s *EngulfingHeikinAshi) OnCandles(ctx context.Context, oneHourCandles []candle.Candle) (signal.Signal, error) {
	...
	case NoPositionState:
		...
		if currCandle.Close > currCandle.Open && // Current candle is bullish
			...
			s.stateMachine.TransitionTo(WaitingForExitSignal, "bullish_engulfing_pattern", position.LongBullish, "buy signal triggered")
			return signal.Signal{
