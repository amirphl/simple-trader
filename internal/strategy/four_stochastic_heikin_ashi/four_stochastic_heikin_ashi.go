// Package four_stochastic_heikin_ashi
package four_stochastic_heikin_ashi
...
// isGreenStochasticAboveOversold checks if the green stochastic (length 100) is above oversold (20)
func (s *FourStochasticHeikinAshi) isGreenStochasticAboveOversold() bool {
	if len(s.stochastic100.K) == 0 {
		return false
	}
...
// isGreenStochasticBelowOversold checks if the green stochastic (length 100) is below oversold (20)
func (s *FourStochasticHeikinAshi) isGreenStochasticBelowOversold() bool {
	if len(s.stochastic100.K) == 0 {
		return false
	}
...
func (s *FourStochasticHeikinAshi) OnCandles(ctx context.Context, oneHourCandles []candle.Candle) (signal.Signal, error) {
	...
	case NoPositionState:
		...
		if s.areAllStochasticsOversold() && s.isHeikinAshiBullish(ha) {
			s.stateMachine.TransitionTo(AllBelowOversold, "all_stochastics_oversold_bullish_ha", position.LongBullish, "long signal triggered")
			return signal.Signal{
				Time:         lastCandle.Timestamp,
		...
		if s.areAllStochasticsOversold() {
			s.stateMachine.TransitionTo(WaitingForBullishHeikenAshiToEnterLong, "all_stochastics_oversold_waiting_bullish_ha", position.Hold, "waiting for bullish heikin ashi to enter long")
		}

	case WaitingForBullishHeikenAshiToEnterLong:
		// Check if Heiken Ashi has become bullish
		if s.isHeikinAshiBullish(ha) {
			s.stateMachine.TransitionTo(AllBelowOversold, "bullish_ha_detected", position.LongBullish, "long signal triggered")
			return signal.Signal{
				Time:         lastCandle.Timestamp,
				Position:     position.LongBullish,
	...
	case AllBelowOversold:
		// Check if green stochastic has crossed above oversold
		if s.isGreenStochasticAboveOversold() {
			s.stateMachine.TransitionTo(WaitingForEitherExitSignalOrAnotherLongSignal, "green_stoch_crossed_above_20", position.Hold, "waiting for exit signal or another long signal")
		}

	case WaitingForEitherExitSignalOrAnotherLongSignal:
		// Check if green stochastic has crossed back below oversold
		if s.isGreenStochasticBelowOversold() {
			s.stateMachine.TransitionTo(NoPositionState, "green_stoch_crossed_below_20", position.LongBearish, "exit signal triggered")
			return signal.Signal{
				Time:         lastCandle.Timestamp,
				Position:     position.LongBearish,
				Reason:       "green stochastic crossed below oversold",
				StrategyName: s.Name(),
				TriggerPrice: lastCandle.Close,
				Candle:       lastCandle,
			}, nil
		...
		if s.isGreenStochasticOverbought() {
			...
			if s.isHeikinAshiBearish(ha) {
				s.stateMachine.TransitionTo(NoPositionState, "green_stoch_overbought_bearish_ha", position.LongBearish, "exit signal triggered")
				return signal.Signal{
					Time:         lastCandle.Timestamp,
					Position:     position.LongBearish,
			...
			s.stateMachine.TransitionTo(WaitingForBearishHeikenAshiToExitPosition, "green_stoch_overbought_bullish_ha", position.Hold, "waiting for bearish heiken ashi to exit position")
		}

	case WaitingForBearishHeikenAshiToExitPosition:
		// Check if Heiken Ashi has become bearish
		if s.isHeikinAshiBearish(ha) {
			s.stateMachine.TransitionTo(NoPositionState, "bearish_ha_detected", position.LongBearish, "exit signal triggered")
			return signal.Signal{
				Time:         lastCandle.Timestamp,
				Position:     position.LongBearish,
